...
1. WXT 扩展入口、content、background、sidepanel 已落地。
2. 手动分析、幽灵模式、Google 登录、钱包连接已可运行。
3. 会员展示与购买入口按免费策略隐藏。
4. Polymarket 持仓读取与结果关联已迁移。

说明后续迁移不再从基础框架开始，应优先围绕剩余可选功能做小范围补齐。
...
5. `Phase 4` 幽灵模式自动分析。
6. `Phase 5` 分析结果面板与交互。
7. `Phase 6` Google 登录与用户态管理。
8. `Phase 7` 免费策略下的会员状态与配额兼容。
9. `Phase 8` 钱包连接与链切换。
10. `Phase 10` Polymarket 持仓读取、展示与分析结果关联。

### 暂停迁移

1. `Phase 9` 会员购买与次卡购买：当前实行免费策略，购买入口暂不面向用户。

### 剩余可选

1. 如恢复收费，再迁移订单创建、链上支付和订单确认。
2. 如需要旧登录兼容，再迁移邮箱 OTP 登录。
3. 上线前补一次真实 Chrome 手动验收和异常兜底清单。
